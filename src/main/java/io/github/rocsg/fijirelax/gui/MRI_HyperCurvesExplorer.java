...
// 
	...
	JButton buttonSwitchMono=new JButton("<Html>Single T2<br>(Mono)<br><Html>");
	
	/** The button switch bi. */
	JButton buttonSwitchBi=new JButton("<Html>Two T2<br>(Bi-exp)<br><Html>");
	...
	/** The button export. */
	JButton buttonCute=new JButton("Cute");

	/** The button export. */
	JButton buttonToggleJoint=new JButton("Tog.join");
...
/** The gaussian weighting. */
	...
	private boolean normalizePDInMiddleTab=false;
		...
		private boolean cute=false;
				...
				public static void main(String[]args) {
					...
					String earScanPath=		"/home/rfernandez/Bureau/A_Test/EARSCAN/BLE_001_S1/MRI/Ble_001_S2_HyperMap step 0.tif";
					ImagePlus img=IJ.openImage(earScanPath);
					img.show();
					new MRI_HyperCurvesExplorer().runExplorer(earScanPath);
			//		new MRI_HyperCurvesExplorer().runExplorerFromHyperMap(new HyperMap(IJ.openImage(earScanPath)));
			...
			//		new MRI_HyperCurvesExplorer().runExplorerFromHyperMap(new HyperMap(IJ.openImage("/home/fernandr/Bureau/test.tif")));
			//		ImagePlus img=IJ.openImage("/home/rfernandez/Bureau/Releases/FijiRelax_DOI/Tutorial_01_First_steps/Input_data/Images/HyperMap_simulated_fantoms.tif");
			...
			//		new MRI_HyperCurvesExplorer().runExplorerFromHyperMap(hyp);
				...
				public void updateSelectionsCurvesAndTimes(){
					if(!cute){
						for(int i=0;i<isSelectedCurve.length;i++)for(int j=0;j<isSelectedCurve[i].length;i++)isSelectedCurve[i][j]=true;
					}
				}		
						...
						for(int z=0;z<dims[2];z++) {
							...
							for(int c=0;c<t1t2trte[t][z].length;c++) {
								if(t1t2trte[t][z][c][0]!=memTr) {
									isSelectedCurve[t][nt1]=isUp;
									if(cute)isUp=!isUp;
									memTr=t1t2trte[t][z][c][0];nt1++;
								}
								int cLen=t1t2trte[t][z].length;
								if(
									(c<cLen-1) && (t1t2trte[t][z][c+1][0]==t1t2trte[t][z][c][0]) 
									||
									(c>0) && (t1t2trte[t][z][c-1][0]==t1t2trte[t][z][c][0]) ){
						...
						for(int z=0;z<dims[2];z++) {
							...
							for(int c=0;c<t2trte[t][z].length;c++) {
								if(t2trte[t][z][c][0]!=memTr) {
									isSelectedCurve[t][nt1]=isUp;
									if(cute)isUp=!isUp;
							memTr=t2trte[t][z][c][0];nt1++;
						}
						int cLen=t2trte[t][z].length;
						if(
							(c<cLen-1) && (t2trte[t][z][c+1][0]==t2trte[t][z][c][0]) 
							||
							(c>0) && (t2trte[t][z][c-1][0]==t2trte[t][z][c][0]) ){
	...
	//		this.t1Times=new double[nTimepoints][dims[2]][];
		...
		public void initializeGUI() {
			...
			if(isBionanoDisplay) {
				...
				buttonCute.setFont(fontGui);
				buttonToggleJoint.setFont(fontGui);
	...
	//		imgCan2.setMagnification(1.0*DX_IMG/dims[0]);
			...
			exportPanel.add(buttonToggleJoint);
			exportPanel.add(new JLabel());
			exportPanel.add(buttonCute);
			...
			buttonCute.addActionListener(this);
			buttonToggleJoint.addActionListener(this);
	...
	*/	        plotT22.setLineWidth(3);        
	...
	public void actualizeDisplayedNumbers() {
			...
			params[1][0].setText(String.format("%5.0f",MRUtils.convertBionanoM0InPercentage(T1T2MixedEstimation ? paramsTimelapseT1T2[this.tCor][0] : paramsTimelapseT1[this.tCor][0],this.normalizePDInMiddleTab ) ) );
			params[1][1].setText(String.format("%5.0f",T1T2MixedEstimation ? paramsTimelapseT1T2[this.tCor][1] : paramsTimelapseT1[this.tCor][1]));
			params[1][4].setText(String.format("%3.3f",T1T2MixedEstimation ? khi2T1T2Mono[this.tCor] : this.khi2T1Mono[this.tCor]));
	
			params[3][0].setText(String.format("%5.0f",MRUtils.convertBionanoM0InPercentage(T1T2MixedEstimation ? (paramsTimelapseT1T2[this.tCor][6]+paramsTimelapseT1T2[this.tCor][7]) : paramsTimelapseT1[this.tCor][0],this.normalizePDInMiddleTab)));
			params[3][1].setText(String.format("%5.0f",T1T2MixedEstimation ? paramsTimelapseT1T2[this.tCor][8] : paramsTimelapseT1[this.tCor][1]));
			params[3][4].setText(String.format("%3.3f",T1T2MixedEstimation ? khi2T1T2Bicomp[this.tCor] : this.khi2T1Mono[this.tCor]));
	/*
	...
	*/
			params[9][0].setText(String.format("%5.0f",MRUtils.convertBionanoM0InPercentage(T1T2MixedEstimation ? paramsTimelapseT1T2[this.tCor][0] : paramsTimelapseT2[this.tCor][0],this.normalizePDInMiddleTab)));
			params[9][1].setText(String.format("%5.0f",T1T2MixedEstimation ? paramsTimelapseT1T2[this.tCor][2] : paramsTimelapseT2[this.tCor][1]));
			params[9][4].setText(String.format("%3.3f",T1T2MixedEstimation ? khi2T1T2Mono[this.tCor] : this.khi2T2Mono[this.tCor]));
	
			params[11][0].setText(String.format("%5.0f",MRUtils.convertBionanoM0InPercentage(T1T2MixedEstimation ? paramsTimelapseT1T2[this.tCor][6] : paramsTimelapseT2[this.tCor][6],this.normalizePDInMiddleTab)));
			params[11][1].setText(String.format("%5.0f",T1T2MixedEstimation ? paramsTimelapseT1T2[this.tCor][9] : paramsTimelapseT2[this.tCor][8]));
			params[11][2].setText(String.format("%5.0f",MRUtils.convertBionanoM0InPercentage(T1T2MixedEstimation ? paramsTimelapseT1T2[this.tCor][7] : paramsTimelapseT2[this.tCor][7],this.normalizePDInMiddleTab)));
			...
			params[11][4].setText(String.format("%3.3f",T1T2MixedEstimation ? this.khi2T1T2Bicomp[this.tCor] : this.khi2T2Bicomp[this.tCor]));
			/*
	...
	*/		
			...
			params[15][4].setText(String.format("%3.3f",T1T2MixedEstimation ? this.khi2T1T2Bionano[this.tCor] : this.khi2T1T2Bionano[this.tCor]));
		...
		public void displayResultsAgain() {
			...
			repaintAll();
			IJ.showMessage("Displayed results again !");
		}
		...
		public void keyTyped(KeyEvent e) {
			...
			if (jointLevel==0 && e.getKeyChar()=='j') {
				jointLevel=1;
			}
			if (jointLevel==1 && e.getKeyChar()=='o') {
				jointLevel=2;
			}
			if (jointLevel==2 && e.getKeyChar()=='i') {
				jointLevel=3;
			}
			if (jointLevel==3 && e.getKeyChar()=='n') {
				jointLevel=4;
			}
			if (jointLevel==4 && e.getKeyChar()=='t') {
				jointLevel=0;
				T1T2MixedEstimation=!T1T2MixedEstimation;
			}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==buttonToggleJoint){
			T1T2MixedEstimation=!T1T2MixedEstimation;
			IJ.showMessage("Switching joint fit to "+T1T2MixedEstimation);
			startPlotsAndRoi();
		}
		if(e.getSource()==buttonCute){
			cute=!cute;
			updateSelectionsCurvesAndTimes();			
			displayResultsAgain();
		}

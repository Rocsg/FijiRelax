...
public class MRUtils  {
	...
	public static ImagePlus[] computeT1T2MapMultiThreadSlices(final ImagePlus[]imgsTemp,ImagePlus mask,double sigmaSmoothing,int fitType,int algType, boolean debug,boolean forgetFirstT2) {
		...
		final double [][]listTrForThreadsForget=forgetLastT2( listTrForThreads ); 
		final double [][]listTeForThreadsForget=forgetLastT2( listTeForThreads ); 
			...
			public void run() {  
				...
				for(int w=0;w<nWork;w++) {
					...
					if(listMask[numProc][w][0]<=0) {
						results[numProc][w]=new double[] {0,0,0,0};
						continue;
					}
		...
		for(int ind=0;ind<fetch.length;ind++) {
			...
			if(fitType==MRUtils.T1_MONO_RICE)mapData[1][z][y*X+x]=(float) results[proc][indexProc][1];
			if(fitType==MRUtils.T2_MONO)mapData[1][z][y*X+x]=(float) results[proc][indexProc][1];
			if(fitType==MRUtils.T2_MONO_BIAS)mapData[1][z][y*X+x]=(float) results[proc][indexProc][1];
			if(fitType==MRUtils.T2_MONO_RICE)mapData[1][z][y*X+x]=(float) results[proc][indexProc][1];
			if(fitType==MRUtils.T1T2_MONO_RICE) {
				mapData[1][z][y*X+x]=(float) results[proc][indexProc][1];
				mapData[2][z][y*X+x]=(float) results[proc][indexProc][2];
			}
			mapData[MRUtils.getNparams(fitType)][z][y*X+x]=(float) results[proc][indexProc][MRUtils.getNparams(fitType)];
		}
	...
	public static Object[]makeFit(final double[]tabTrTimesTemp, final double[]tabTeTimesTemp, double[]tabData,int fitType,int algType,int nbIter,double sigma,boolean debug){
		...
		if(algType==SIMPLEX) {
			...
			if(fitType==T2_MONO_BIAS)estimatedParams[0]+=estimatedParams[2];
		}	                                                
		else{
	...
	public static double convertBionanoM0InPercentage(double m0) {
		return (m0/maxM0BionanoForNormalization);
	}
    ...
    public static final int T2_MONO =19; // exponentielle decroissante : M0 exp(-te/T2)
    public static final int T2_MONO_BIAS =20; // exponentielle decroissante : M0 exp(-te/T2) + bias
    public static final int T2_MONO_RICE =21; // exponentielle decroissante : M0 exp(-te/T2)

    public static final int T2_MULTI_RICE=22; // exponentielle decroissante à deux composantes : M01 exp(-te/T21) + M02 exp(-te/T22)

    public static final int T1_MONO = 10; //exponentielle convergente vers M0 :  M0 (1- exp(-tr/T1))
    public static final int T1_MONO_BIAS = 10; //exponentielle convergente vers M0 :  M0 (1- exp(-tr/T1))
    public static final int T1_MONO_RICE = 11; //exponentielle convergente vers M0 :  M0 (1- exp(-tr/T1))
	public static final int T1T2_MONO_BIAS = 24; //Cycle T1-T2 a une composante : M0 exp(-te/T2) (1- exp(-tr/T1))
	public static final int T1T2_MONO = 23; //Cycle T1-T2 a une composante : M0 exp(-te/T2) (1- exp(-tr/T1))
	public static final int T1T2_MONO_RICE = 25; //Cycle T1-T2 a une composante : M0 exp(-te/T2) (1- exp(-tr/T1))
	public static final int T1T2_MULTI_RICE = 26; //Cycle T1-T2 a deux T2 : ( M01exp(-te/T21) + M02exp(-te/T22) ) (1- exp(-tr/T1))
	public static final int T1T2_BIONANO = 29; //Calcul de T1 par la méthode des polynomes, calcul de T2 par la méthode des deux points
	public static final int T1T2_DEFAULT_T2_MONO_RICE=30;//Amendement de T1T2 avec un delta T2 de 25 ms
	public static final int T1T2_DEFAULT_T2_MULTI_RICE=31;//Amendement de T1T2 avec un delta T2 de 25 ms
	...
	public static final double maxDisplayedPDratio=1.2;
